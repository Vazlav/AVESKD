...
Процедура УстановитьНачальныеНастройкиПоМетаданнымРегистра(ИмяРегистра)
	...
	// + САВ (05.08.2010)
	ДобавитьВТекстЗапросаВидыДокументов(ОписаниеПолейВыборки, ОписаниеОтборовРезультатаЗапроса);	
	// - САВ (05.08.2010)
...
Процедура УстановитьПредставленияПолей(СтруктураПредставлениеПолей, ПостроительОтчета) Экспорт
	...
	// + САВ (05.08.2010)
	СтруктураПредставлениеПолей.Вставить("ВидДокумента", "Вид документа");
	// - САВ (05.08.2010)
...
Функция ВосстановитьНастройки(Объект, Отчет, Форма, ИмяОбъекта = Неопределено) Экспорт
	...
	СтруктураНастройки.Вставить("Пользователь", ПараметрыСеанса.ТекущийСотр);
...
Процедура УправлениеОтображениемЭлементовФормы(Объект, Форма, КнопкаИмя = Неопределено) Экспорт
	...
	Если Форма.ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Найти("ЗаголовокОтчета") <> Неопределено Тогда
...
Процедура ФормаПриОткрытии(Объект, Отчет, Форма, СоставПанелиДополнительныхЭлементовФормы = Неопределено, ИмяОбъекта = Неопределено) Экспорт
	...
	Если Объект.мВосстанавливатьНастройкиПриОткрытии Тогда
		...
		//Если ЗначениеЗаполнено(ПараметрыСеанса.ТекущийСотр) И Не ЗначениеЗаполнено(Объект.ДатаНач) Тогда
		//	
...
Процедура ДобавитьВТекстЗапросаВидыДокументов(ОписаниеПолейВыборки, ОписаниеОтборовРезультатаЗапроса, ИмяТаблицыИсточника = "ИсточникДанных", ИмяПоляДокумента = "Регистратор") Экспорт
	...
	Для Каждого Документ Из Метаданные.Документы Цикл
		...
		Если НЕ ПустаяСтрока(ИмяРегистра) И НЕ Документ.Движения.Содержит(Метаданные.РегистрыНакопления[ИмяРегистра]) Тогда
			Продолжить;
		КонецЕсли;
		
		ОписаниеПолейВыборки = ОписаниеПолейВыборки + "		
			|	КОГДА " + ИмяТаблицыИсточника + "." + ИмяПоляДокумента + " ССЫЛКА Документ." + Документ.Имя + "
			|		ТОГДА &" + Документ.Имя;
			
		ОписаниеОтборовРезультатаЗапроса = ОписаниеОтборовРезультатаЗапроса + "		
			|	КОГДА " + ИмяТаблицыИсточника + "." + ИмяПоляДокумента + " ССЫЛКА Документ." + Документ.Имя + "
			|		ТОГДА &" + Документ.Имя;
			
		ПостроительОтчета.Параметры.Вставить(Документ.Имя, Справочники.ВидыДокументов.НайтиПоКоду(Документ.Имя));
...
мСтруктураПериодов.Вставить("ВидДокумента", 2);
